...
**Command-line build (e.g. CI):** Use a local DerivedData path so the build can write without system DerivedData permissions:
`xcodebuild -scheme "Phoenix Control" -configuration Debug build -derivedDataPath PhoenixControl/build`
Then open `PhoenixControl/build/Build/Products/Debug/Phoenix Control.app`.
