...
final class ExecutiveDashboardLauncher {
    ...
    /// Launch streamlit run dashboard.py with REPO_PATH; open browser on success; on failure return error message.
    func launch(repoPath: String, completion: @escaping (Result<Void, String>) -> Void) {
        let expanded = (repoPath as NSString).expandingTildeInPath
        guard !expanded.isEmpty else {
            completion(.failure("Repo path is empty."))
            return
        }
        var isDir: ObjCBool = false
        guard FileManager.default.fileExists(atPath: expanded, isDirectory: &isDir), isDir.boolValue else {
            completion(.failure("Repo path does not exist or is not a directory."))
            return
        }
        ...
        guard FileManager.default.fileExists(atPath: dashboardPy.path) else {
            completion(.failure("dashboard.py not found at repo root. Expected: \(dashboardPy.path)"))
            return
        }
        ...
        do {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) { [weak self] in
                if process.isRunning {
                    ...
                    self?.process = nil
                    completion(.failure("Executive Dashboard process exited before opening. Check that streamlit is installed: pip install streamlit"))
                }
            }
        } catch {
            completion(.failure("Failed to start Executive Dashboard: \(error.localizedDescription). Ensure streamlit is installed: pip install -r requirements-dashboard.txt"))
        }
    }
