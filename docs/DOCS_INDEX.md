...
| **Run tests / understand test suite** | [Test suite (document all)](#test-suite-document-all): **use project venv** (`.venv/bin/python` or `source .venv/bin/activate`) so fast pytest and gates pass; how to run (local + CI), markers, workflows, full file list, fixtures, [FULL_REPO_TEST_SUITE_PLAN.md](./FULL_REPO_TEST_SUITE_PLAN.md). Fast run: `PYTHONPATH=. .venv/bin/python -m pytest tests/ -m "not slow" -x` → ~253 passed, 16 deselected. |
...
| **Systems test** | [scripts/systems_test/run_systems_test.py](../scripts/systems_test/run_systems_test.py) — Phases 1–7; optional sim run. Use project venv: `PYTHONPATH=. .venv/bin/python scripts/systems_test/run_systems_test.py --all` |
...
**Content completeness checkers updated (2026-03-06):** [scripts/book_script_content_validation.py](../scripts/book_script_content_validation.py) (`--check-v2-slots`) and [scripts/ci/content_coverage_report.py](../scripts/ci/content_coverage_report.py) (step 5, `--no-v2-slots` to skip) both know about `chapter_thesis`/`chapter_thread`/`chapter_permission` arc fields and `pivot` field on STORY atoms. Run `--check-v2-slots` to see which arcs still need these fields populated; use CHAPTER_THESIS_BANK.md machine-readable lookup tables to auto-populate.
**Translation awareness updated (2026-03-06):** [scripts/translate_atoms_all_locales_cloud.py](../scripts/translate_atoms_all_locales_cloud.py) docstring and [config/localization/quality_contracts/README.md](../config/localization/quality_contracts/README.md) both document the 4 new slot types (PIVOT/TAKEAWAY/THREAD/PERMISSION), arc thesis translation requirements, glossary priorities, and golden regression requirements for v2 content.
...
Qwen draft → Comparator/Judge vs English source → automated repair loop (max_loops, config-driven) → pass or manual_review. No Claude at runtime. No human in the repair loop. All patches applied automatically by `PatchApplier`. Manual review sections written to a structured packet; `manual_review_queue.json` feeds PhoenixControl Manual Review tab (sorted by severity).

**Design decisions locked:**
- `max_loops`: config-driven, schema max=5; hard gate failure after max_loops → `manual_review` only (never auto-pass)
- Aggregate pass: all hard gates pass AND scored_total/max_scored_total ≥ 0.75 (locale-overridable)
- TTS readability: **hard gate** — unshippable audiobook content is a hard fail
- Judge: Qwen-max, temperature=0.1, rotated seed per loop, separate system prompt — committed, not open
- Parallel: `asyncio.Semaphore(max_parallel_sections=6)`; sections within a book are embarrassingly parallel
- Schema binding: `comparator_result_v2.schema.json` version=2.0 must match checklist schema_version=2.0

| Item | File |
|------|------|
| **Spec** | [docs/AUDIOBOOK_PIPELINE_SPEC.md](./AUDIOBOOK_PIPELINE_SPEC.md) — canonical reference: purpose, full flow, all 9 gates in prose, scoring logic, patch injection mechanics, parallel architecture, artifact contracts, manual review protocol, roles, gap tracker, config table, locale support, schema version binding |
| **Step 0 — Polish rubric** | [config/audiobook_script/static_polish_rubric.yaml](../config/audiobook_script/static_polish_rubric.yaml) — TTS cadence rules (tts_c1–c5), psychological impact rules (psy_p1–p5), narrative flow rules (flow_f1–f4), compliance rules; authored once offline; feeds checklist `rubric_ref` |
| **Master runtime config** | [config/audiobook_script/comparator_config.yaml](../config/audiobook_script/comparator_config.yaml) — max_loops (range [1,5]), parallel (max_parallel_sections, max_parallel_books), judge model (committed: qwen-max temp=0.1 rotated seed), patch injection strategy, token budget, scoring thresholds, locale overrides, manual review packet spec, observability |
| **Checklist (9 gates)** | [config/audiobook_script/comparison_checklist_v2.yaml](../config/audiobook_script/comparison_checklist_v2.yaml) — semantic_fidelity (hard), claim_integrity (hard), psychological_safety (hard), tts_readability_cadence (hard), compliance_disclaimer_preservation (hard); emotional_arc_alignment (scored w=2.0), native_regional_language_fit (scored w=2.5), narrative_flow_cohesion (scored w=1.5), polish_emotional_impact (scored w=2.0); locale overrides for zh-TW/HK/SG/CN/ja-JP/ko-KR |
| **Result schema** | [schemas/comparator_result_v2.schema.json](../schemas/comparator_result_v2.schema.json) — version-aligned to checklist v2.0; `checklist_schema_version` binding enforced per gate result; schema fail → manual_review (never silent pass) |
| **Loop script** | [scripts/audiobook_script/run_comparator_loop.py](../scripts/audiobook_script/run_comparator_loop.py) — async parallel sections, `PatchApplier` (hard patches first, auto-injected, no human), artifact trace per loop, manual review packet writer, `manual_review_queue.json` appender, batch summary, JSONL observability log |
| **Go-live checklist** | [docs/GO_LIVE_FINAL_CHECKLIST.md](./GO_LIVE_FINAL_CHECKLIST.md) — 10 items: freeze/rollback, CI enforcement, safety caps, judge quality, golden regression gate, observability + PhoenixControl Manual Review tab, cost/rate protection, secrets hygiene, evidence pack, operator runbook |

**Artifacts at runtime:**
- `artifacts/audiobook/{batch_id}/{book_id}/{section_id}/loop_{n}/trace.json` — per-loop trace
- `artifacts/audiobook/{batch_id}/batch_summary.json` — batch-level pass/fail summary
- `artifacts/audiobook/loop_decisions.jsonl` — observability JSONL (all loop decisions)
- `artifacts/audiobook/manual_review_queue.json` — **high-visibility** PhoenixControl Manual Review feed

**Still to implement before go-live:**
- `prompts/draft_audiobook_v2.txt` — Qwen draft system prompt
- `prompts/judge_audiobook_v2.txt` — Qwen judge system prompt (references checklist gates + rubric)
- `config/audiobook_script/golden_regression_set/` — fixed multilingual sections (TW/HK/SG/CN required)
- `scripts/audiobook_script/run_regression.py` — golden set regression runner (CI gate)
- `docs/audiobook_operator_runbook.md` — full operator runbook (Item 10)
...
**Single report:** Run [scripts/ci/content_coverage_report.py](../scripts/ci/content_coverage_report.py) from repo root. It aggregates atoms coverage (STORY + non-STORY), plan-time coverage_check (K-table + pool sizes), teacher readiness per teacher, and Pearl Prime v2 slot readiness (PIVOT/TAKEAWAY/THREAD/PERMISSION arc fields + STORY atom `pivot` field). Writes `artifacts/content_coverage_report.json` and prints a one-page summary. Exit 1 if any content is missing.

**For v2 slot detail only:** Run `python scripts/book_script_content_validation.py --check-v2-slots` — checks `chapter_thesis` / `chapter_thread` / `chapter_permission` fields in master_arcs and `pivot` field on STORY atoms. See [docs/CHAPTER_THESIS_BANK.md](./CHAPTER_THESIS_BANK.md) machine-readable lookup tables to auto-populate arc fields.
...
| **Content coverage report** | [scripts/ci/content_coverage_report.py](../scripts/ci/content_coverage_report.py) | `artifacts/content_coverage_report.json` + stdout summary; `--no-teachers` to skip teacher checks; `--no-v2-slots` to skip v2 arc field check |
| **Book script content validation** | [scripts/book_script_content_validation.py](../scripts/book_script_content_validation.py) — all locs/langs, story pools, non-STORY slots, teacher readiness, system content, v2 slot arc fields | `--check-v2-slots` to include Pearl Prime v2 fields; `--json` for machine-readable output |
| **Pearl Prime v2 slot readiness** | `scripts/book_script_content_validation.py --check-v2-slots` — chapter_thesis (20/arc), chapter_thread (20/arc), chapter_permission (high-cost intents), pivot field on STORY atoms | Arc gap list + pivot field coverage; see [CHAPTER_THESIS_BANK.md](./CHAPTER_THESIS_BANK.md) lookup tables |
...
**How to get fast pytest green locally (253 passed, 16 deselected):** Use the project venv. Without it, `pip`/`python` may be missing or point to the wrong env. Run: `PYTHONPATH=. .venv/bin/python -m pytest tests/ -v -m "not slow" -x` from repo root. Ensure deps are installed in the venv: `.venv/bin/pip install -r requirements-test.txt` (and `requirements.txt` if running gates). See [SYSTEMS_V4.md](./SYSTEMS_V4.md) § systems test and [artifacts/freebies/EVIDENCE.md](../artifacts/freebies/EVIDENCE.md) for venv usage with gates.

### How to run

**Environment (venv — use this so tests and gates pass):** Use the project venv so `python`, `pytest`, PyYAML, and jsonschema resolve. From repo root: `source .venv/bin/activate` then run the commands below with `python`, or use `PYTHONPATH=. .venv/bin/python -m pytest ...` (and same for other scripts) without activating. If you see "command not found: pip" or wrong Python, create/use venv: `python3 -m venv .venv` then `.venv/bin/pip install -r requirements-test.txt` (and `-r requirements.txt` if running pipeline/gates).

| Command | What runs |
|---------|-----------|
| `PYTHONPATH=. .venv/bin/python -m pytest tests/ -v --tb=short` | All tests. Run from repo root; use `.venv` so deps resolve. |
| `PYTHONPATH=. .venv/bin/python -m pytest tests/ -m "not slow" -x` | **Fast set (core-tests equivalent):** excludes slow; ~253 passed, 16 deselected. Used by core-tests CI. |
| `PYTHONPATH=. .venv/bin/python -m pytest tests/ -v --tb=short` | All tests (same as first; default collection). |
| `PYTHONPATH=. .venv/bin/python -m pytest tests/ -m sanity` | Sanity checks only (config load, registry consistency); quick feedback. |
| `PYTHONPATH=. .venv/bin/python -m pytest tests/ -m "sanity or intelligent"` | Robust/intelligent tests only (no slow). See [ROBUST_INTELLIGENT_TESTING.md](./ROBUST_INTELLIGENT_TESTING.md). |
| `PYTHONPATH=. .venv/bin/python -m pytest tests/ -m slow` | Slow tests only (atoms coverage 100%, teacher E2E smoke). |
| `PYTHONPATH=. .venv/bin/python -m pytest tests/test_arc_loader.py -v` | Single file. |
| `.venv/bin/pip install -r requirements-test.txt` | Install pytest, pyyaml, jsonschema, feedparser in venv (required before running). |

**Environment:** Run from repo root. `PYTHONPATH=.` (or repo root on `PYTHONPATH`) required so `phoenix_v4` and other packages resolve. **Use `.venv`** (activate or `.venv/bin/python`) so the fast pytest run and production readiness gates pass. Optional: `ATOMS_ROOT` to override atoms directory.
...
| **Run systems test** | [scripts/systems_test/run_systems_test.py](../scripts/systems_test/run_systems_test.py) — Phases 1–7; use `.venv/bin/python` so PyYAML/deps resolve |
...
| [scripts/ci/content_coverage_report.py](../scripts/ci/content_coverage_report.py) | **Content coverage report** — single report: atoms (STORY + non-STORY), plan coverage_check, teacher readiness, Pearl Prime v2 slot readiness (PIVOT/TAKEAWAY/THREAD/PERMISSION arc fields + pivot on STORY atoms); writes artifacts/content_coverage_report.json + one-page summary; `--no-v2-slots` to skip v2 check |
...
| [scripts/ci/content_coverage_report.py](../scripts/ci/content_coverage_report.py) | Content coverage — atoms + plan + teacher readiness + Pearl Prime v2 slot readiness (PIVOT/TAKEAWAY/THREAD/PERMISSION); single report; `--no-v2-slots` to skip | ✓ |
| [scripts/book_script_content_validation.py](../scripts/book_script_content_validation.py) | Book script content validation — all locs/langs, story pools, non-STORY slots, teacher readiness, system content, v2 arc fields; `--check-v2-slots` for Pearl Prime v2 | ✓ |
| [scripts/audiobook_script/run_comparator_loop.py](../scripts/audiobook_script/run_comparator_loop.py) | Qwen-Only Audiobook Comparator Loop — async parallel sections, automated PatchApplier, artifact trace, manual review packet, manual_review_queue.json feed | ✓ |
| [config/audiobook_script/comparator_config.yaml](../config/audiobook_script/comparator_config.yaml) | Audiobook pipeline master config — max_loops, parallel, judge model, patch injection, scoring, token budget, locale overrides, observability | ✓ |
| [config/audiobook_script/comparison_checklist_v2.yaml](../config/audiobook_script/comparison_checklist_v2.yaml) | 9-gate comparison checklist — 5 hard gates (incl. TTS), 4 scored gates; rubric_ref; locale overrides; aggregate threshold | ✓ |
| [config/audiobook_script/static_polish_rubric.yaml](../config/audiobook_script/static_polish_rubric.yaml) | Static polish rubric — TTS cadence, psychological impact, narrative flow, compliance; authored offline; feeds checklist rubric_ref | ✓ |
| [schemas/comparator_result_v2.schema.json](../schemas/comparator_result_v2.schema.json) | JSON schema for judge output; version-bound to checklist schema_version=2.0; schema fail routes to manual_review | ✓ |
| [docs/GO_LIVE_FINAL_CHECKLIST.md](./GO_LIVE_FINAL_CHECKLIST.md) | 10-item production go-live checklist for audiobook pipeline | ✓ |
| [docs/AUDIOBOOK_PIPELINE_SPEC.md](./AUDIOBOOK_PIPELINE_SPEC.md) | Canonical spec for Qwen-Only Audiobook Pipeline — 15 sections; single reference for all roles | ✓ |
