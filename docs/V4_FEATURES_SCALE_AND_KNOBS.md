...
- **Stage 6 — Book renderer** — Renders CompiledBook → prose output (manuscript/QA). `phoenix_v4.rendering`: prose_resolver (atom_id → prose from atoms/, compression_atoms, teacher_banks), book_renderer (TxtWriter, render_book). QA: `scripts/render_plan_to_txt.py` uses Stage 6. Pipeline: `--render-book`, `--render-formats txt`, `--render-dir`; outputs to `artifacts/rendered/<plan_id>/book.txt`. Edge cases: placeholders/silence → [Placeholder/Silence: TYPE]; missing atoms → fail or [Missing: atom_id]; persona/topic from plan or inferred from first STORY atom_id via topic_engine_bindings.
