...
def _load_mapping(config_root: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    path = config_root / "sdg_news_topic_mapping.yaml"
...
def classify_sdgs(
    ...
    topic_to_sdg, keyword_to_topic = _load_mapping(config_root)
    ...
    for item in items:
        ...
        topic = _classify_text(text, keyword_to_topic) or DEFAULT_TOPIC
        item["topic"] = topic

        mapping = topic_to_sdg.get(topic) or topic_to_sdg.get(DEFAULT_TOPIC) or {}
