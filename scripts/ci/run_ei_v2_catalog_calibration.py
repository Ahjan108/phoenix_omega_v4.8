...
Output: artifacts/ei_v2/calibration_*.json
...
def main() -> int:
    ...
    out_path = Path(args.out) if args.out else REPO_ROOT / "artifacts" / "ei_v2" / f"calibration_{datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}.json"
