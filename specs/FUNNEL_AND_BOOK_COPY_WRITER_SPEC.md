...
| **Key turning point (Teacher Mode)** | Book author provides raw story with teacher | Copywriter shapes; author highlights teacher | Editor/author approve | Teacher bank `approved_atoms/` for that teacher; see TEACHER_MODE_AUTHORING_PLAYBOOK. |
...
**Constraint list for authors:** ... (keep this at top of file)
...
**Before:** One sentence — specific situation (e.g. profession, context).

**Practice:** One sentence with a physical detail (what they did, what they felt in the body).

**After:** One sentence with specific, measurable or observable change.
```

**Exact reference:** See `funnel/burnout_reset/stories/burnout.md` in the repo for the full example. The system pulls the body of the `## story_id` block into Email 3; the **Before / Practice / After** labels can be used in the file for structure, and the rendered email may use just the paragraph text. Total word count for the story block (Before + Practice + After): **120–150 words**.
...
**Teacher Mode** = books that are built around a **real teacher’s teachings** (a real person’s method or lineage). The pipeline doesn’t invent the teacher; it **assembles** content from that teacher’s approved material. The **key turning point story** is the **book author’s** own story (e.g. Nihala’s) about the moment or arc when **they** were changed by **that teacher’s** work — so the **author** is the narrator, and the **teacher** is the one being credited and highlighted. Where it goes: in the **teacher’s content bank**, in **approved story atoms** for that teacher. Exact steps: see **TEACHER_MODE_AUTHORING_PLAYBOOK.md** in this repo (onboard teacher, plan book, fill gaps, approve atoms).
...
- **Teacher bank** for that teacher: under `SOURCE_OF_TRUTH/teacher_banks/<teacher_id>/approved_atoms/` (or the path your team uses for teacher-scoped content). The “author story” or “key turning point” slot is defined in the Teacher Mode structural spec; the playbook describes how to add and approve atoms. See **TEACHER_MODE_AUTHORING_PLAYBOOK.md** in `specs/` — sections on onboarding a teacher, planning a Teacher Mode book, and filling gaps (approved_atoms, doctrine). Use **book author** as the voice; for Teacher Mode, **author highlights teacher** in that story.
